...
export const revalidateDestination: CollectionAfterChangeHook<Destination> = async ({
  doc,
  previousDoc,
  req: { payload, context },
}) => {
...
export const revalidateDelete: CollectionAfterDeleteHook<Destination> = async ({
  doc,
  req: { payload, context }
}) => {
  if (!context.disableRevalidate) {
    // Revalidar la página del post y la lista principal de posts
    const pathsToRevalidate = ['/', '/destino']

    await sendRevalidationRequest(payload, pathsToRevalidate, ['destinations'])
  }
  return doc
} 
