...
export const revalidateTour: GlobalAfterChangeHook = async ({
    doc,
    previousDoc,
    req: { payload, context },
}) => {
    if (!context.disableRevalidate) {
        ...
        const tagsToRevalidate = ['tours-sitemap', 'toursPage']

        // Revalidar el tour específico y la página principal de tours
        if (doc._status === 'published') {
            // Revalidar la página individual del tour
            payload.logger.info(`Solicitando revalidación de tour en ruta: tourPage`)
